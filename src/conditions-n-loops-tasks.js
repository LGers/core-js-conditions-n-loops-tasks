...
function convertToRomanNumerals(num) {
  ...
  temp = Math.floor((num - temp * 10) % 11);
...
function isPalindrome(str) {
  ...
  return isPalindrome(sliceHeadAndTail(str));
}
...
function isContainNumber(num, digit) {
  ...
  if (Math.floor(num / 10) < 1) return false;

  res = isContainNumber(Math.floor(num / 10), digit);
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix[0].length; j += 1) {
      ...
      arr[j][matrix.length - 1 - i] = initialMatrix[i][j];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  return !curIteration
    ? result
    : buffer[(iterations % curIteration || curIteration) - 1];
}
